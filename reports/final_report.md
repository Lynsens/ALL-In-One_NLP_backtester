...
There are two goals of visualization. First, explaining graphically why the user-designed model (in the above demo, our `dummy_model`) made that decision. Second, the possible profit/loss profile with the decisions made by the user-designed model in a real market environment.

The visualization of the market simulation is partially developed based on the original built-in visualization within the `Backtestering.py` package. However, the limitation of `Backtestering.py` come with the fact that it being a backtesting library: where only when, where, and how much buys and sells are interested, but not why such trading decisions were made.

By implementing our modification to `Backtestering.py` plotting methods, we are now able to simultaneously represent some user-embedded model indicators. In this case, we are tracking the `high` (a numerical representation of detected positive semantics), `low` (a numerical representation of detected negative semantics), and  `indictor` (a function of the pervious two indicators). So that if the model made some undesired decision upon any point, the user may embed some tracking indicators into the model, then visualize these indicators alongside the trading decisions generated by the model. This will greatly facilitate the experiences of debugging/tuning the model, and also improves the model's interpretability to a certain extent — not to mention the fact this is a fairly cool demonstration tool which might impress your audiences.

As all the data are aligned by time series. A user can easily track the data on a certain day with a mouse and then examine the accuracy and efficiency of the trading decisions the trading model made in a synchronized sense. This makes model's evaluation not just an output of a revenue number or some printouts, but a holistic experience that you can seamlessly checking the model's ability to catch the "trend" of the market, if the indicators are working as expected, if the overall revenue is gaining at what rate, etc.
